...
Ext.ux.tree.ColumnTree = Ext.extend(Ext.tree.TreePanel, {
    ...
    onRender : function(){
		...
		if(this.innerCt.getWidth() > this.headerWidth) hW = this.innerCt.getWidth();
		this.innerCt.setWidth(hW);
		this.headers.setWidth(hW + scrollOffset);
    },
	onResize : function() {
		Ext.tree.ColumnTree.superclass.onResize.apply(this, arguments);
		var scrollOffset = 19;
		var hW = this.headerWidth;
		if(this.getWidth() > this.headerWidth) hW = this.getWidth();
		this.innerCt.setWidth(hW - scrollOffset);
